...
function App() {
    ...
    // Effect to control SOS button visibility based on active ride status
    useEffect(() => {
        if (activeRide) {
            // In a real app, you would check the actual ride status and timing
            // For this demo, we'll simulate showing SOS when activeRide exists
            setShowSOS(true);
        } else {
            setShowSOS(false);
        }
    ...
    return (
        <div className={`app-container ${darkMode ? 'dark-mode' : ''}`}>
            <Router>
                ...
                {/* SOS Button - Only shown during active rides */}
                {isLoggedIn && showSOS && (
